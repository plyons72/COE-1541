...
{
    struct trace_item *tr_entry=malloc(sizeof(struct trace_item)); //allocate a trace_item size space with malloc
    size_t size;
    ...
    if (argc == 1) { //determine argument inputs and assign
    ...
    printf("\nHow many instructions do you want to generate: "); //get number of instructions from user
    scanf("%d \n", &trcount );
    
    for (i = 0 ; i < trcount ; i++) { //for each instruction, enter their parameters
    ...
    trace_init();
    while(1) { //write trace information to a file
    ...
    trace_uninit(); //uninitialize the trace
