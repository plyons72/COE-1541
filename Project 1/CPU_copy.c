...
int main(int argc, char **argv)
{
    ...
    int prediciton_method = 0; //branch prediction on or off. 0 for predict not taken, 1 for 1-bit prediction. Default value is 0
    ...
    if (argc == 3) {
        ...
        if (prediction_method < 0 || prediction_method > 1 || trace_view_on < 0 || trace_vew_on > 1) {
            fprintf(stdout, "\nInvalid input arguments\n\n");
            exit(0);
        }
    }
    ...
    while(1) { //Execute the trace program
        ...
        if (trace_view_on) {/* print the executed instruction if trace_view_on=1 */
            ...
            /*break;
             ...
             break;
             case ti_RTYPE:
             printf("\t[%s Stage] RTYPE:", stage_ID) ;
             printf(" (PC: %x)(sReg_a: %d)(sReg_b: %d)(dReg: %d) \n", tr_entry[i]->PC, tr_entry[i]->sReg_a, tr_entry[i]->sReg_b, tr_entry[i]->dReg);
             break;
             case ti_ITYPE:
             printf("\t[%s Stage] ITYPE:", stage_ID)) ;
             printf(" (PC: %x)(sReg_a: %d)(dReg: %d)(addr: %x)\n", tr_entry[i]->PC, tr_entry[i]->sReg_a, tr_entry[i]->dReg, tr_entry[i]->Addr);
             break;
             case ti_LOAD:
             printf("\t[%s Stage] LOAD:", stage_ID)) ;
             printf(" (PC: %x)(sReg_a: %d)(dReg: %d)(addr: %x)\n", tr_entry[i]->PC, tr_entry[i]->sReg_a, tr_entry[i]->dReg, tr_entry[i]->Addr);
             break;
             case ti_STORE:
             printf("\t[%s Stage] STORE:", stage_ID)) ;
             printf(" (PC: %x)(sReg_a: %d)(sReg_b: %d)(addr: %x)\n", tr_entry[i]->PC, tr_entry[i]->sReg_a, tr_entry[i]->sReg_b, tr_entry[i]->Addr);
             break;
             case ti_BRANCH:
             printf("\t[%s Stage] BRANCH:", stage_ID)) ;
             printf(" (PC: %x)(sReg_a: %d)(sReg_b: %d)(addr: %x)\n", tr_entry[i]->PC, tr_entry[i]->sReg_a, tr_entry[i]->sReg_b, tr_entry[i]->Addr);
             break;
             case ti_JTYPE:
             printf("\t[%s Stage] JTYPE:", stage_ID)) ;
             printf(" (PC: %x)(addr: %x)\n", tr_entry[i]->PC,tr_entry[i]->Addr);
             break;
             case ti_SPECIAL:
             printf("\t[%s Stage] SPECIAL:", stage_ID)) ;
             break;
             case ti_JRTYPE:
             printf("\t[%s Stage] RTYPE:", stage_ID)) ;
             printf(" (PC: %x) (sReg_a: %d)(addr: %x)\n", tr_entry[i]->PC, tr_entry[i]->dReg, tr_entry[i]->Addr);
